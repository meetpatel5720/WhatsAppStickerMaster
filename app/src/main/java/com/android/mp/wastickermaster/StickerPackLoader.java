...
import static com.android.mp.wastickermaster.StickerPackProvider.ANDROID_APP_DOWNLOAD_LINK_IN_QUERY;
import static com.android.mp.wastickermaster.StickerPackProvider.IOS_APP_DOWNLOAD_LINK_IN_QUERY;
import static com.android.mp.wastickermaster.StickerPackProvider.LICENSE_AGREENMENT_WEBSITE;
import static com.android.mp.wastickermaster.StickerPackProvider.PRIVACY_POLICY_WEBSITE;
import static com.android.mp.wastickermaster.StickerPackProvider.PUBLISHER_EMAIL;
import static com.android.mp.wastickermaster.StickerPackProvider.PUBLISHER_WEBSITE;
import static com.android.mp.wastickermaster.StickerPackProvider.STICKER_FILE_EMOJI_IN_QUERY;
import static com.android.mp.wastickermaster.StickerPackProvider.STICKER_FILE_NAME_IN_QUERY;
import static com.android.mp.wastickermaster.StickerPackProvider.STICKER_PACK_ICON_IN_QUERY;
import static com.android.mp.wastickermaster.StickerPackProvider.STICKER_PACK_IDENTIFIER_IN_QUERY;
import static com.android.mp.wastickermaster.StickerPackProvider.STICKER_PACK_NAME_IN_QUERY;
import static com.android.mp.wastickermaster.StickerPackProvider.STICKER_PACK_PUBLISHER_IN_QUERY;
...
public class StickerPackLoader {
    ...
    public static ArrayList<StickerPack> fetchStickerPacks(Context context) throws IllegalStateException {
        final Cursor cursor = context.getContentResolver().query(StickerPackProvider.AUTHORITY_URI, null, null, null, null);
        if (cursor == null) {
    ...
    private static List<Sticker> getStickersForPack(Context context, StickerPack stickerPack) {
        ...
        for (Sticker sticker : stickers) {
            final byte[] bytes;
            try {
                bytes = fetchStickerAsset(stickerPack.identifier, sticker.stickerFileName, context.getContentResolver());
                if (bytes.length <= 0) {
                    throw new IllegalStateException("Asset file is empty, pack: " + stickerPack.name + ", sticker: " + sticker.stickerFileName);
                }
                sticker.setSize(bytes.length);
            } catch (IOException | IllegalArgumentException e) {
                throw new IllegalStateException("Asset file doesn't exist. pack: " + stickerPack.name + ", sticker: " + sticker.stickerFileName, e);
            }
        }
        return stickers;
    }
    ...
    private static List<Sticker> fetchFromContentProviderForStickers(String identifier, ContentResolver contentResolver) {
        ...
        if (cursor != null && cursor.getCount() > 0) {
            ...
            do {
                ...
                stickers.add(new Sticker(Arrays.asList(emojisConcatenated.split(",")),name));
            } while (cursor.moveToNext());
        }
        if (cursor != null) {
            cursor.close();
    ...
    private static Uri getStickerListUri(String identifier) {
        return new Uri.Builder().scheme(ContentResolver.SCHEME_CONTENT).authority(BuildConfig.CONTENT_PROVIDER_AUTHORITY).appendPath(StickerPackProvider.STICKERS).appendPath(identifier).build();
    }

    public static Uri getStickerAssetUri(String identifier, String stickerName) {
        return new Uri.Builder().scheme(ContentResolver.SCHEME_CONTENT).authority(BuildConfig.CONTENT_PROVIDER_AUTHORITY).appendPath(StickerPackProvider.STICKERS_ASSET).appendPath(identifier).appendPath(stickerName).build();
    }
}
